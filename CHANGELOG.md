...
- **Multi-agent packaging.** Skills and always-on docs now live once in
  ...
  unchanged, so existing installs migrate transparently on this `/plugin update`.
  `sync-plugin.sh` was absorbed into `sync-agents.sh`, and `bump-version.sh` now
  stamps the Codex/Gemini manifests too.
...
- **Codex CLI** (`packages/codex/`) and **Gemini CLI** (`packages/gemini/`)
  ...
  Codex `plugin.json`/`config.toml` and a Gemini `gemini-extension.json`.
  **Experimental:** the one-command installs depend on two packaging details
  (Codex manifest schema, Gemini subdir install + `${extensionPath}`) that are
  not yet smoke-tested against live CLIs — each package README flags them. The
  Codex manual path (`~/.agents/skills/` + `config.toml`) is verified.
- Maintainer guide `docs/multi-agent-packaging.md` (architecture, sync workflow,
  how to add a new agent); design rationale in the `documentations` repo.
